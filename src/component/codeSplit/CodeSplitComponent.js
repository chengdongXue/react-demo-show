...
export default class WasteRenderComponent extends Component {
  ...
  // 为了避免 React 组件中的渲染浪费，我们将挂钩到shouldComponentUpdate生命周期方法
  shouldComponentUpdate(propsNext, stateNext) {
    // shouldComponentUpdate 方法是一个生命周期方法，当React渲染一个组件时，这个方法不会被调用 ，并根据返回值来判断是否要继续渲染组件
    ...
    // 检查了当前状态对象this.state.count中的计数值是否等于 === 到下一个状态nextState.count对象的计数值
    // 如果它们相等，则不应该重新渲染，因此我们返回false，如果它们不相等则返回true，因此应该重新渲染以显示新值
  ...
  // 该组件的初始状态为 {count: 0} 。当我们单击click Me 按钮时，它将count状态设置为 1。屏幕的 0 就变成了1。
  // 当我们再次单击该按钮时出现了问题，组件不应该重新呈现，因为状态没有更改。count的上个值为1，新值也1，因此不需要更新 DOM
  ...
  render() {
    ...
    return (
      <div style={{
        width: '100%', height: '100px', backgroundColor: 'green', marginTop: 20,
      }}
      >
